...
- **Cheap frozen-import smoke test (`zonos-smoke-test.yml`).** A full Zonos release build is ~50 min and ~2.9 GB, far too expensive to use as the feedback loop for the one-line `numpy<2.4` pin in `packaging/requirements.txt`. That pin exists because the frozen engine once reported no GPU with `torch/CUDA unavailable: cannot load module more than once per process` — numpy 2.4+'s guard, tripped when PyInstaller's bootstrap removes numpy in `cleanup_loaded_modules` and torch re-imports it (issue #77). The smoke test reproduces **only that import path**, cheaply: on `windows-latest` with **no GPU**, it `uv pip install`s just `torch==2.4.1` (cu124) + `numpy<2.4` + `pyinstaller` (no zonos, no torchaudio, no weights, no bundle), PyInstaller-freezes a tiny `import torch` + cuda-probe script (`packaging/smoke_import.py` via `packaging/smoke-import.spec`, which mirrors the real spec's torch collection so the frozen import path is faithful), runs the frozen exe, and **fails if the output contains the double-load string** (or the exe crashes). `torch.cuda.is_available()` returning False on the GPU-less runner is fine: the double-load surfaces at import, before any CUDA call. It runs on `workflow_dispatch` and on PRs that touch `requirements.txt`, the spec, the synthesizer, or the smoke files, so the `numpy<2.4` pin can be validated in minutes before paying for a full release build.
