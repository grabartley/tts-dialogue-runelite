...
1. **Wrong/missing race, gender, or origin accent** (e.g. a woman voiced male, a talkable monster with no race, a foreigner sounding like the locals) -> `tools/overrides.json`.
   - An override **replaces** the table entry, so it must carry `race` + `gender`. `race` must be one of Human, Elf, Dwarf, Goblin, Gnome, Monkey, Troll, Undead, Demon, Wizard; map a wiki race that is not one of these (Dorgeshuun -> Goblin, Vampyre -> Undead, Imp -> Demon, gorilla/ape -> Monkey, ...). A truly unmappable species (Serpent, Merfolk, ...) gets a `byId` style only.
